...
export const addTodoApp = (req: requestInterface, res: responseInterface) => {
    ...
    if (todoList.length > 0){
        ...
        while (salirDelLoop === false && i < todoList.length){
            if (todoList && todoList[i].task === req.body.task){
                salirDelLoop = true;
                res.end("Esta tarea ya existe en la lista")

            }else if (i === todoList.length -1){
                salirDelLoop = true;
                let nuevoTodo = {"task": req.body.task,"completado": req.body.completado}
